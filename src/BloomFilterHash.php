...
class BloomFilterHash
{
     ...
     */
    public function JSHash(string $string, ?int $len = null): int
    {
        $hash        = 1315423911;
     ...
     */
    public function PJWHash(string $string, ?int $len = null): int
    {
     ...
     */
    public function ELFHash(string $string, ?int $len = null): int
    {
        $hash        = 0;
        $len || $len = strlen($string);
        for ($i = 0; $i < $len; ++$i) {
     ...
     */
    public function BKDRHash(string $string, ?int $len = null): int
    {
     ...
     */
    public function SDBMHash(string $string, ?int $len = null): int
    {
        $hash        = 0;
        $len || $len = strlen($string);
        for ($i = 0; $i < $len; ++$i) {
     ...
     * 它是有史以来发布的最有效的哈希函数之一。
     */
    public function DJBHash(string $string, ?int $len = null): int
    {
        $hash        = 5381;
        $len || $len = strlen($string);
        for ($i = 0; $i < $len; ++$i) {
     ...
     */
    public function DEKHash(string $string, ?int $len = null): int
    {
        $len || $len = strlen($string);
        $hash        = $len;
        for ($i = 0; $i < $len; ++$i) {
     ...
     */
    public function FNVHash(string $string, ?int $len = null): int
    {
